	...
	//$conn = "";
	$server_path = "http://discovermypet.in/pet/";
	...
	function Encrypt($password, $data)
	{
	   ...
	   /*   $salt = substr(md5('poo', true), 8);
		...
		$ct = mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $data, MCRYPT_MODE_CBC, $iv);
		 ...
		 $encrypt = base64_encode('Salted__' . $salt . $ct);
		
		$ent =  base64_encode('Salted__' . $salt . $ct);  */
	...
	//$conn=mysqli_connect($db_host,$db_user,$db_password) or DIE('Connection to host is failed, perhaps the service is down!');
	...
	function get_service_category($user_id)
	{
		...
		$sql = "SELECT id,service_master,service_img FROM tbl_service_master WHERE city_id = '".$row_city_id['id']."'and id !='0' order by id";
		$result = mysqli_query($link,$sql);
		if(mysqli_num_rows($result) != 0)
		{	
			$i=0;
	...
	function city_wise_service_master($city_id)
	{
		...
		$sql = "SELECT id,service_master,service_img FROM tbl_service_master ";
		$result = mysqli_query($link,$sql);
		if(mysqli_num_rows($result) != 0)
		{	
			$i=0;
	...
	function get_service_category_details($cat_id,$city_name)
	{
		...
		{	
			...
			$app_list = array("success" => 1,"result" => $app_info,"icon_img"=>$row_banner["service_img"],"banner"=>$banner);
		}
		else
			$app_list = array("success" => 0, "error" => 1);
	...
	function forget_pwd($email)
	{
		...
		{	
			...
			<p style='font-size: 20px; text-align: center;'>Click here <a href='http://discovermypet.in/pet/reset_pwd.php?email=$email1'>Open Link</a> </p>
			</body>
			</html>
			";
	...
	function fb_register($name,$email,$img,$latitude,$longitude)
	{
		global $link;
		...
		else
		{
			...
			if($error == "")
			{
				$sql = "INSERT into app_users (name, email,password,city,pincode,address1,address2,latitude,longitude, created_date,is_active) values ('".$name."', '".$email."', '".$password."','".$city."','".$pincode."','".$address1."','".$address2."','".$latitude."','".$longitude."','".date('Y-m-d')."','1')";
							
				$to = $email;
				$subject = "PassWord of Discovermypet";

				$message = "
				<html>
				<head>
				<title>PassWord of Discovermypet</title>
				</head>
				<body style='background-color: #efefef;height: 335px;'>		
				<img style='margin-left: 100px;padding-top: 20px;' src='http://discovermypet.in/pet/uploads/dmp_logo.__orange.png'/>
				<div style='background-color: #fff;height: 150px;margin-bottom: -64px;margin-left: 100px;width: 577px;' >
				<p style='color:rgb(149,33,52);font-size:24px;padding-top:32px;text-align:center'>Welcome to DiscoverMyPet </p>
				<p style='text-align: center; font-size: 20px;'>Your PassWord :  $password  </p>
	...
	function get_tips_news()
	{
		global $link;
		$i=0;
		$sql = "SELECT id,news_title,news_desc,news_img,news_link,DATE_FORMAT(created_date,'%d-%m-%Y') as created_date FROM tbl_tips_news";
		$result = mysqli_query($link,$sql);
		if(mysqli_num_rows($result) > 0)
		{
			while($row = mysqli_fetch_array($result))
			{
				$short_desc= substr($row["news_desc"],0,25)." more...";
				
				$app_info[$i] = array("id" => $row["id"],"news_title" => $row["news_title"],"news_desc" => $row["news_desc"],"desc_short" => $short_desc,"news_img" => $row["news_img"],"news_link" => $row["news_link"],"created_date" => $row["created_date"]);
				$i++;
			}
			$app_list = array("success" => 1,"result" => $app_info);
			return $app_list;
	...
	function send_verification_code($name, $email, $mobile)
	{	
		global $link;	
		...
		{	
			$code = rand(1000, 9999); 
			//$mobile = "91".$mobile;
			$sql = "select email from app_users where email = '".$email."' ";
			$result = mysqli_query($link,$sql);
			//echo mysqli_num_rows($result);
			if(mysqli_num_rows($result) == 0)
			{
				$value = array("success" => 0, "error" => "Email Does not Exist");
			}
			else
			{
				$sql = "UPDATE app_users SET verify_code = '".$code."',mobile= '".$mobile."' WHERE email = '".$email."' ";
				mysqli_query($link,$sql);
				
				$sql_id = "SELECT id FROM app_users WHERE email = '".$email."' and mobile= '".$mobile."' and name= '".$name."'";
				...
				if(mysql_error() == "")				
				{
					...
					$value = array("success" => 1, "error" => 0,"id"=>$id);
				}		
				else
	...
	{		
		...
		{
			$sql = "UPDATE app_users SET image ='http://discovermypet.in/pet/uploads/socialprofile/default-user-image.png',verify_code = '',is_active ='1' WHERE email = '".$email."' and mobile= '".$mobile."'";
			mysqli_query($link,$sql);
			$value = array("success" => 1, "error" => 0);
		}	
		return $value ;
	...
	{		
		...
		{ 
			...
			$img = "http://discovermypet.in/pet/$path";	
	...
	{		
		...
		{		
			...
			$path = "uploads/pet_pics/$imagename";	 	
			$image1 = "http://discovermypet.in/pet/$path";			
			...
			$image_thub = "http://discovermypet.in/pet/$path_thub";			
	...
	function most_like_order()
	{
		...
		while($row_like = mysqli_fetch_array($result_like))
		{
			...
			$app_info[$i] =  array("image" => $row_img["post_img"],"rank" => $i);
			$i++;
		}
		$app_list = array("success" => 1,"result" => $app_info);
		return $app_list;
	...
	function homescreen($user_id,$latitude,$longitude)
	{
		...
		else    // Location is not present so buy service showing
		{
			...
			if($row_notification = mysqli_fetch_array($result_notification) > 0)
			{
				$app_list = array("success" => 1,"result" => $app_info,"notification"=>$count,"flag"=>"1");
			}
			else
			{
				$app_list = array("success" => 1,"result" => $app_info,"notification"=>0,"flag"=>"1");
		...
		while($row_request = mysqli_fetch_array($result_request))
		{
			 ...
			 if($row_request["type"] == "request")
			{
				...
				if(mysqli_num_rows($result_frd_acc)>0)
				{
					...
					$app_info[$i] =  array("Friend_id"=> $row_request["user_id"],"usre_name" => $row_name["name"],"user_image" => $row_name["image"],"request_status"=>"Accepted","text"=>$text,"time"=> $posted_when,"type"=>$row_request["type"]);
				}
				
				else
				{
					$text = $row_name["name"]." sent friend request";
					$app_info[$i] =  array("Friend_id"=> $row_request["user_id"],"usre_name" => $row_name["name"],"user_image" => $row_name["image"],"request_status"=>"No Action","text"=>$text,"time"=> $posted_when,"type"=>$row_request["type"]);
				}
				$i++;
			} 
			else if($row_request["type"] == "response") 
			{
				...
				$app_info[$i] =  array("Friend_id"=> $row_request["user_id"],"usre_name" => $row_name["name"],"user_image" => $row_name["image"],"text"=>$text,"time"=> $posted_when,"type"=>$row_request["type"]);
				$i++;
			} 
			else if($row_request["type"] == "like")
			{
				...
				{
					$app_info_like[$j] =  array("Friend Id"=> $row_request["user_id"],"Name" => $row_name["name"],"User Image" => $row_name["image"],"Timeline_id" => $row_img["id"],"Post Image" => $row_img["post_img"],"text"=>"Liked your photo","When"=> $posted_when,"type"=>$row_request["type"],"comments"=>$count_cmt,"likes"=>$count_like,"Post Type"=>"image","like_status"=>"1");
				$j++;
				}
				else
				{
					$app_info_like[$j] =  array("Friend Id"=> $row_request["user_id"],"Name" => $row_name["name"],"User Image" => $row_name["image"],"Timeline_id" => $row_img["id"],"Post Image" => $row_img["post_img"],"text"=>"Liked your photo","When"=> $posted_when,"type"=>$row_request["type"],"comments"=>$count_cmt,"likes"=>$count_like,"Post Type"=>"image","like_status"=>"0");
				$j++;
				}			
			}
			else
			{
				...
				if(mysqli_num_rows($result_user_like) != 0)
				{
					$app_info_like[$j] =  array("Friend Id"=> $row_request["user_id"],"Name" => $row_name["name"],"User Image" => $row_name["image"],"Timeline_id" => $row_img["id"],"Post Image" => $row_img["post_img"],"text"=>"Comment on your photo","When"=> $posted_when,"type"=>$row_request["type"],"comments"=>$count_cmt,"likes"=>$count_like,"Post Type"=>"image","like_status"=>"1");
					$j++;
				}
				else
				{
					$app_info_like[$j] =  array("Friend Id"=> $row_request["user_id"],"Name" => $row_name["name"],"User Image" => $row_name["image"],"Timeline_id" => $row_img["id"],"Post Image" => $row_img["post_img"],"text"=>"Comment on your photo","When"=> $posted_when,"type"=>$row_request["type"],"comments"=>$count_cmt,"likes"=>$count_like,"Post Type"=>"image","like_status"=>"0");
					$j++;
				}
			}		
		}	
		if(empty($app_info) and empty ($app_info_like))
		{
			$app_list = array("success" => 1,"requestandresponse" => "no records found" ,"likeandcomment" => "no records found" ,"count" =>$count1);
		}
		else if(empty($app_info))
		{
			$app_list = array("success" => 1,"requestandresponse" => "no records found" ,"likeandcomment" =>  $app_info_like ,"count" =>$count1);
		}
		else if(empty($app_info_like))
		{
			$app_list = array("success" => 1,"requestandresponse" => $app_info ,"likeandcomment" =>  "no records found" ,"count" =>$count1);
		}
		else
		{
				$app_list = array("success" => 1,"requestandresponse" => $app_info ,"likeandcomment" =>  $app_info_like ,"count" =>$count1);
		}
		
		$sql_update = "UPDATE tbl_notification SET is_read = 1 WHERE user_id = '".$user_id."' ";
		$result_update =mysqli_query($link,$sql_update);
		return $app_list;
	} 
	...
	function timeline_profile($user_id)
	{
		...
		while($row_friend = mysqli_fetch_array($result_friend))
		{
			...
			while($row_breed = mysqli_fetch_array($result_breed))
			{
				...
				$app_info[$i] =array("name" => $row_user["name"],"image" => $row_user["image"],"friend"=>$row_friend["friend_id"],"breed"=>$breed,"walk"=>$walk,"status"=>$row_user["status"]);
				$i++;
		}
	...
	{		
		global $link;
		$i=0;
		$sql="SELECT doctor_name,contact_no,dr_address,dr_clinic_name,dr_time,dr_time1,dr_img,dr_specification,dr_experience,dr_qulification from app_users  WHERE id = '".$user_id."' ";
		$result = mysqli_query($link,$sql);
		while($row = mysqli_fetch_array($result))
		{
			if($row["doctor_name"] =="")
			...
			{
				$app_info[$i] = array("doctor_name" => $row["doctor_name"],"contact_no" => $row["contact_no"],"dr_address" => $row["dr_address"],"dr_clinic_name" => $row["dr_clinic_name"],"dr_time" => $row["dr_time"],"dr_time1" => $row["dr_time1"],"dr_img" => $row["dr_img"],"dr_specification" => $row["dr_specification"],"dr_experience" => $row["dr_experience"],"dr_qulification" => $row["dr_qulification"]);
				$i++;
			}
	...
	function edit_my_doctor_ios($user_id,$doctor_name,$contact_no,$dr_address,$dr_clinic_name,$dr_time,$dr_time1)
	{		
		global $link;
		...
		$img = "http://discovermypet.in/pet/$path_img";
					
		$sql="UPDATE app_users SET doctor_name = '".$doctor_name."',contact_no = '".$contact_no."',dr_address = '".$dr_address."',dr_clinic_name = '".$dr_clinic_name."',dr_img = '".$img."',dr_time = '".$dr_time."',dr_time1 = '".$dr_time1."' WHERE id = '".$user_id."' ";
		$result = mysqli_query($link,$sql);
		if(mysql_error() == "")		
			$returnarr = array("success" => 1, "error" => "0");
		else
	...
	function edit_my_doctor_andriod($user_id,$doctor_name,$contact_no,$dr_address,$dr_clinic_name,$dr_time,$dr_time1,$dr_img,$dr_specification,$dr_experience,$dr_qulification)
	{		
		global $link;
		$imagename = $user_id.".png";
		...
		$sql="UPDATE app_users SET doctor_name = '".$doctor_name."',contact_no = '".$contact_no."',dr_address = '".$dr_address."',dr_clinic_name = '".$dr_clinic_name."',dr_img = '".$img."',dr_time = '".$dr_time."',dr_time1 = '".$dr_time1."',dr_specification = '".$dr_specification."',dr_experience = '".$dr_experience."',dr_qulification = '".$dr_qulification."' WHERE id = '".$user_id."' ";
		$result = mysqli_query($link,$sql);
		if(mysql_error() == "")		
			$returnarr = array("success" => 1, "error" => "0");
		else
			...
			while($row_product = mysqli_fetch_array($result_product))
			{
				...
				if($row_main_cat["main_category"] == "Pet Food")
				{
					if($city =="pune")
					{
						...
						{
							$sql_weight_nm = "SELECT weight_name FROM tbl_pet_weight WHERE id = '".$row_attribute['weight_id']."'";
							$result_weight_nm = mysqli_query($link,$sql_weight_nm);
							$row_weight_nm = mysqli_fetch_array($result_weight_nm);
							
							$attributes[$a] = array("atr_id"=>$row_attribute["id"],"weight_name"=>$row_weight_nm["weight_name"],"size_name"=>$row_attribute["size_name"],"price"=>$row_attribute["price"],"img"=>$row_attribute["img"]);
							$a++;
						}
						...
						$app_info[$i] = array("id"=>$row_product["prod_id"],"category_id"=>$row_product["category_id"],"category_name"=>$row_product["catogories_name"],"plan_name"=>$row_product["plan_name"],"description"=>$row_product["description"],"rate"=>$row_product["rate"],"orignal_price"=>$row_product["orignal_price"],"offer"=>$row_product["offer"],"image"=>$image);
						
						$app_info[$i]["attribute"] = $attributes;				
						$app_info[$i]["color"] = $colors_nm;				
						$i++;
	...
	function get_product_details($product_id)
	{
		...
		$sql ="select s.id,s.category_id, s.plan_name, s.description, s.bullet_title, s.bullet_points, s.rate, s.rate_title, s.orignal_price, s.offer, s.image, c.catogories_name FROM tbl_service_plans as s, tbl_product_category as c where s.category_id = c.id and s.service_id = 2 $subquery ";
		$result = mysqli_query($link, $sql);
		while($row = mysqli_fetch_array($result))
		{		
	...
	function add_to_cart($add_cart,$user_id)
	{
		...
		{	
			...
			$attribute_name = $arr['Order'][$i]['attribute_name'];		 
			$rate = $arr['Order'][$i]['rate'];		 
			$attribute_img = $arr['Order'][$i]['attribute_img'];		 
			
			/* $attr_id = explode(",",$attribute_id);
			for($a=0;$a<count($attr_id);$a++)
			{
				$sql_attribute = "SELECT atr_id,price FROM tbl_product_attribute WHERE id = '$attr_id[$a]' and product_id = '$id'";
				$result_attribute = mysqli_query($link,$sql_attribute);
				$row_attribute = mysqli_fetch_array($result_attribute);	
				
				if($row_attribute["atr_id"] =='2')
				{
		...
		if($type=="product")
			{
				...
				$sql = "INSERT into tbl_cart (product_id,pack_id,quantity,type,cart_master_id,rate,attribute_name,attribute_img,user_id) VALUES('$id','$pack_id','$quantity','$type','$cart_master_id','$rate','$attribute_name','$attribute_img','$user_id')"; 
				//$result = mysqli_query($link,$sql);
	...
	function view_cart($user_id)
	{
		...
		global $link;
		$sql_order = "SELECT id,DATE_FORMAT(date_purchase,'%d-%m-%Y') as date_purchase,is_paid FROM tbl_cart_master WHERE user_id = '".$user_id."' order by date_purchase desc";	
		...
		{  
			...
			$sql_cart = "SELECT product_id,package_id,pack_id,quantity,attribute_name,rate FROM tbl_cart WHERE cart_master_id = '".$row_order['id']."'";	
			...
			{	
				$total=0;	
				if(($row_cart['package_id'])==0)
				{
					$k = 0;
					
				/* 	 if(!empty($row_cart['attribute_id']))
					{
						$att_id = explode(",",$row_cart['attribute_id']);
						for($a=0;$a<count($att_id);$a++)
						{
							$sql_attribute = "SELECT id,price,img,atr_id,attribute_name FROM tbl_product_attribute WHERE id = '$att_id[$a]'";
							$result_attribute = mysqli_query($link,$sql_attribute);
							$row_attribute = mysqli_fetch_array($result_attribute);
							$row_attribute["attribute_name"];
							$attribute[$a] =$row_attribute["attribute_name"]; 						
						}	
				//			$attribute = implode(",",$attribute);
						$total += $row_cart["quantity"]*$row_cart["rate"];
						...
						$app_info_plan[$j] = array("type"=>"product","id" => $row_cart["product_id"],"pack_id"=>$row_cart["pack_id"],"category_id" => $row_package_name["category_id"],"category_name" => $row_category["catogories_name"],"plan_name" => $row_package_name["plan_name"],"description" => $row_package_name["description"],"rate" => $row_cart["rate"],"rate_title" => $row_cart["rate"],"offer" => $row_package_name["offer"],"orignal_price" => $row_package_name["orignal_price"],"image" => $row_attribute["img"],"quantity" => $row_cart["quantity"],"total"=>$total,"attribute_name"=> $attribute); 		
					} 
					else
					{ */
						$sql_pack = "SELECT id,pack,rate as pack_rate FROM tbl_product_pack WHERE product_id = '".$row_cart['product_id']."'";
						$result_pack = mysqli_query($link, $sql_pack);				
						...
						$total_all = $total_all + $total;
						$app_info_plan[$j] = array("type"=>"product","id" => $row_cart["product_id"],"pack_id"=>$row_cart["pack_id"],"category_id" => $row_package_name["category_id"],"category_name" => $row_category["catogories_name"],"plan_name" => $row_package_name["plan_name"],"description" => $row_package_name["description"],"rate" => $row_package_name["rate"],"rate_title" => $row_package_name["rate_title"],"offer" => $row_package_name["offer"],"orignal_price" => $row_package_name["orignal_price"],"image" => $row_package_name["image"],"quantity" => $row_cart["quantity"],"attribute_name" => $row_cart["attribute_name"],"total"=>$total); 				
				...
				elseif($row_cart['product_id']==0)
				{
					
					$sql_package_name = "select id,plan_name,image,description,offer,bullet_title,bullet_points,rate_title,rate FROM tbl_service_plans  WHERE id = '".$row_cart['package_id']."'";	
					$result_package_name = mysqli_query($link, $sql_package_name);
					$row_package_name = mysqli_fetch_array($result_package_name);
					...
					$arrPets[$j] = explode('&*!@' ,$row_package_name["bullet_points"]);		
					//$app_info_point[$k] = array("bullet_points" => $arrPets[$j]);
					
					$app_info_plan[$j] = array("type"=>"package","id" => $row_cart["package_id"],"plan_name" => $row_package_name["plan_name"],"image" => $row_package_name["image"],"description"=>$row_package_name["description"],"offer" => $row_package_name["offer"],"rate" => $row_package_name["rate"],"rate_title" => $row_package_name["rate_title"],"bullet_title" => $row_package_name["bullet_title"],"quantity" => $row_cart["quantity"],"bullet_points" => $arrPets[$j],"total"=>$total);
		...
		//	print_r($app_info_plan);
			...
			{		
				...
				{
					$app_info[$i] = array("id" => $row_order["id"],"all_total"=>$total_all,"order_status"=>"successful","date_purchase"=>$row_order["date_purchase"],"order_detaile"=>$app_info_plan);
					$i++; 						
				}
				else if($row_order["is_paid"]==0)
				{
					continue;
					/* $app_info[$i] = array("id" => $row_order["id"],"all_total"=>$total_all,"order_status"=>"pending","date_purchase"=>$row_order["date_purchase"],"order_detaile"=>$app_info_plan);	
					$i++; 		 */
				}
				else
				{
					
					$app_info[$i] = array("id" => $row_order["id"],"all_total"=>$total_all,"order_status"=>"failed","date_purchase"=>$row_order["date_purchase"],"order_detaile"=>$app_info_plan);	
		...
		{			
			...
			$timeline_id = mysqli_insert_id($link);		
			 ...
			 if(!empty($_FILES['myFile1']['tmp_name'])) 
			 {
				...
				$video = "http://discovermypet.in/pet/$path";	
	...
	{		
		...
		while($row = mysqli_fetch_array($result))
		{
		 $sql_user_name = "select id,name,image from app_users where id = '".$row["user_id"]."'";
			...
			if(mysqli_num_rows($result_user_like) != 0)
			{
				$app_info[$i] = array("id" => $row["id"],"user name" => $row_user_name["name"],"user image" => $row_user_name["image"],"post_img" => $row["post_img"],"user_id" => $row["user_id"],"comment"=>$count_cmt,"like"=>$count_like,"like_status"=>"1","When"=>$posted_when);
				$i++;
			}
			else
			{
				$app_info[$i] = array("id" => $row["id"],"user name" => $row_user_name["name"],"user image" => $row_user_name["image"],"post_img" => $row["post_img"],"user_id" => $row["user_id"],"comment"=>$count_cmt,"like"=>$count_like,"like_status"=>"0","When"=>$posted_when);
				$i++;
			}		
		}
		if (empty($app_info)) 
	...
	//	$app_info =array;
		...
		*POWER(SIN(($origLon-longitude)*pi()/180/2),2))) 
		as distance FROM tbl_service_details WHERE 
		...
		$img = "http://discovermypet.in/pet/uploads/socialprofile/default-user-image.png";
	...
	{	
		...
		$img = "http://discovermypet.in/pet/uploads/socialprofile/default-user-image.png";
		$query = "SELECT id,name,clinic_name,address1,address2,city,phone_no, latitude, longitude, 3956 * 2 * 
		ASIN(SQRT( POWER(SIN(($origLat - latitude)*pi()/180/2),2)
		+COS($origLat*pi()/180 )*COS(latitude*pi()/180)
		*POWER(SIN(($origLon-longitude)*pi()/180/2),2))) 
		as distance FROM tbl_service_details WHERE 
		...
		*POWER(SIN(($origLon-longitude)*pi()/180/2),2))) 
		...
		while($row = mysqli_fetch_assoc($result)) 
		{
			...
			else
			{
				...
				if(mysqli_num_rows($result_frd_req) != 0)
				{
					$app_info[$i] = array("id" => $row["id"],"friend_name" => $row["name"], "image" => $row["image"], "address" => $row["address2"], "mobile" => $row["mobile"], "latitude" => $row["latitude"], "longitude" => $row["longitude"],"request"=>1); 		
					$i++;
				}
				else
				{
					$app_info[$i] = array("id" => $row["id"],"friend_name" => $row["name"], "image" => $row["image"], "address" => $row["address2"], "mobile" => $row["mobile"], "latitude" => $row["latitude"], "longitude" => $row["longitude"],"request"=>0); 		
					$i++;
				}
			}
	...
	{		
		...
		{	
			...
			{
				$sql_user="UPDATE app_users SET image ='http://discovermypet.in/pet/uploads/socialprofile/default-user-image.png' WHERE id = '".$user_id."' ";
				$result_user = mysqli_query($link,$sql_user);
			}
		...
		switch ($_POST["action"])
		{
			...
			case "send_verification_code":
					if(!empty($_POST["key"]))
					{
						...
						if($txt)
						{
							...
							{
								$value = send_verification_code($_POST["name"], $_POST["email"],$_POST["mobile"]); 
							}
							else
							{
			...
			case "edit_my_doctor_ios":
					if(!empty($_POST["key"]))
					{
						...
						if($txt)
						{
							$value = edit_my_doctor_ios($_POST["user_id"],$_POST["doctor_name"],$_POST["contact_no"],$_POST["dr_address"],$_POST["dr_clinic_name"],$_POST["dr_time"],$_POST["dr_time1"]);
						}
						else
						{
							$value = "Sorry..Something went wrong..!!";						
			...
			case "edit_my_doctor_andriod":
					if(!empty($_POST["key"]))
					{
						...
						if($txt)
						{
							$value = edit_my_doctor_andriod($_POST["user_id"],$_POST["doctor_name"],$_POST["contact_no"],$_POST["dr_address"],$_POST["dr_clinic_name"],$_POST["dr_time"],$_POST["dr_time1"],$_POST["dr_img"],$_POST["dr_specification"],$_POST["dr_experience"],$_POST["dr_qulification"]);
						}
						else
						{
							$value = "Sorry..Something went wrong..!!";						
